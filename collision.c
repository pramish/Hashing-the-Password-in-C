...
void gen_random (char *s, const int len)
{
	...
	char array[]= "0123456789!\"#$%&'()*+-/. ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"; 
	for (int i = 0; i < len; ++i) {
        s[i] = array[rand() % (sizeof(array) - 1)];
	...
	for(j = 0; j < 10000; j++)
	{
		...
		if(match == 1) //check the match found. 
		{
			...
			/* When match found print the two strings i.e. the initial string and the randomly generated string which have the same hash values*/
				...
				printf("Random string is %s", rand_str);

							printf("\n");

			/* Print the hash value of the randomly generated string in hexadecimal format */
				printf("Random hash is ");
					for(int k=0; rand_hash[k] != '\0'; k++)
					{
						printf("%x", rand_hash[k]);
					}
					printf("\n");
					
				printf("\nThe number of iteration used is %d",j);	
				break;
...
//If the random string's hash matches with the ones in the repository, match becomes positive
		if(match >= 0)
		{
			...
			/* When match found from the repositary, print the two strings i.e. the random string and the repositary string which have the same hash values*/
				printf("\ncondition2 Random string is %s",rand_str);

					printf("\n");
					printf("\ncondition2 The repository string is %s", repo[10000]);
				break;
			}

			/* Print the hash value of the randomly generated string in hexadecimal format */
				printf("\ncondition2 Random hash is ");
					for(int k=0; rand_hash[k] != '\0'; k++)
					{
						printf("%x", rand_hash[k]);
					}
						printf("\n");
				printf("\ncondition2 The number of iteration used is %d",j);
